...
We can simplify the representations of the above transformations with a single equation as:
...
Sinusoidal encoding was proposed in the paper Attention is All You Need. They proposed to generate $p_i$ using the sinusoidal function:
$$
\begin{cases}
...
where $p_i,2_t$ is the $2th$ element of the d-dimensional vector $p$. The wavelengths form a geometric progression from $2π$ to $10000 · 2π$. 

Many people do not know sinusoidal encoding was suggested so the model can learn to attend to relative positions. Yes, relative positions! This is mostly because people do not read papers with "attention". Let us take an example with $d=2$. Plug sinusoidal encoding into equation 3:
...
Let us calculate the position encoding values for $position=pos$:
...
\begin{align*}
    \sin(pos+k) &= \sin(pos)\sin(k) + \cos(pos)\cos(k) \\
                &= PE_{pos, 0} \sin(k) + PE_{pos, 1} \cos(k) \\ \\
    \cos(pos+k) &= \cos(pos)\cos(k) - \sin(pos)\sin(k) \\
                &= PE_{pos, 1} \cos(k) - PE_{pos, 0} \sin(k) \\
\end{align*}
$$

We can combine the above two equations in a nice matrix notation:
